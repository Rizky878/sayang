...
*/
const XPTN = '𝚁𝙸𝚉𝙺𝚈𝙱𝙾𝚃'; // Nama Bot Whatsapp
    ...
    axios.get(`http://inyourdream.herokuapp.com/glitch?kata1=${teks1}&kata2=${teks2}`).then((res) => {
      imageToBase64(res.data.status)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[ BOTLENT ] Sedang diproses⏳ silahkan tunggu sebentar', MessageType.text, { quoted: m })
...
let err = fs.readFileSync('mp3/' + 'tes2' + '.mp3')
...
if (text.includes('bot')) {
 var nomor = m.participant
 const options = {
       text: `apa kontol manggil manggil  ketik .help lah @${nomor.split("@s.whatsapp.net")[0]}, Mau masukin bot ini ke grup kalian?ketik .creator/.owner`,
       contextInfo: { mentionedJid: [nomor] }
 }
...
let err = fs.readFileSync('mp3/' + 'output' + '.mp3')
 conn.sendMessage(id, err, MessageType.audio, { ptt: true, quoted: m })
}
else if (text == 'kangcopet'){
...
//Tolonglah bro jangan di ubah ubah BANGLENT
